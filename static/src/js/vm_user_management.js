...
publicWidget.registry.VmUserManagement = publicWidget.Widget.extend({
    ...
    events: {
        ...
        'change select[name="default_vm_user_type"]': '_onDefaultTypeChange',
    },
    ...
    _updateAccessWarnings: function (stats) {
        ...
        var $alert = $(`
            <div class="alert ${alertClass} access-coverage-alert mt-3" role="alert">
                ...
                ${accessCoverage < 50 ? '<br><small>Consider using "Assign Default Access" to give VM access to more users.</small>' : ''}
            </div>
        `);
    ...
    _onAutoAssignChange: function (ev) {
        ...
        var $defaultTypeField = this.$('select[name="default_vm_user_type"]').closest('.o_setting_box');
    ...
    _initializeTooltips: function () {
        ...
        this.$('field[name="vm_rental_manager_users"]').attr('title',
            'Users who can create VMs, manage pricing, and access reports');
        this.$('field[name="vm_rental_admin_users"]').attr('title',
            'Users with full system access including hypervisor management');
    },
    ...
    // Метод для валидации пользователей перед сохранением
    validateUserAssignments: function() {
        var managerUsers = this.$('field[name="vm_rental_manager_users"]').val() || [];
        var adminUsers = this.$('field[name="vm_rental_admin_users"]').val() || [];

        // Проверяем пересечения
        var intersection = managerUsers.filter(x => adminUsers.includes(x));
        if (intersection.length > 0) {
            this._showNotification('warning', 'User Role Overlap',
                'Some users are assigned as both Managers and Administrators. Admin role will take precedence.');
        }

        // Проверяем минимальное количество администраторов
        if (adminUsers.length === 0) {
            this._showNotification('warning', 'No VM Administrators',
                'Consider assigning at least one VM Administrator for system management.');
        }
...
var VmUserStatsController = publicWidget.registry.VmUserStatsController = publicWidget.Widget.extend({
    // Этот контроллер будет добавлен в Python как веб-контроллер
});
